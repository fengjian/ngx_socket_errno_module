...
static ngx_int_t ngx_http_upstream_intercept_errors(ngx_http_request_t *r,
    ngx_http_upstream_t *u);
static ngx_int_t ngx_http_upstream_test_connect(ngx_connection_t *c, ngx_peer_connection_t *pc);
    ...
    ngx_http_upstream_t *u, ngx_connection_t *c)
{
    ...
    if (ngx_http_upstream_test_connect(c, &u->peer) != NGX_OK) {
        ngx_http_upstream_next(r, u, NGX_HTTP_UPSTREAM_FT_ERROR);
        return;
    }
    ...
    ngx_uint_t do_write)
{
    ...
    if (!u->request_sent && ngx_http_upstream_test_connect(c, &u->peer) != NGX_OK) {
        ngx_http_upstream_next(r, u, NGX_HTTP_UPSTREAM_FT_ERROR);
        return;
    }
...
static ngx_int_t
ngx_http_upstream_test_connect(ngx_connection_t *c, ngx_peer_connection_t *pc)
{
    int        err;
    socklen_t  len;
